#include "feixi_mujoco_control/feixi_mujoco_hardware.hpp"

#include "feixi_mujoco_control/feixi_pinocchio_dynamics.hpp"
...
MujocoJointActuationMode parse_joint_actuation_mode(
    rclcpp::Logger logger, std::unordered_map<std::string, std::string> const& pmap) {
    auto const it = pmap.find("mujoco_joint_actuation");
    if (it == pmap.end() || it->second.empty()) {
        return MujocoJointActuationMode::Position;
    }
    std::string v;
    v.reserve(it->second.size());
    for (unsigned char uc : it->second) {
    ...
    if (v == "velocity" || v == "vel" || v == "speed") {
        return MujocoJointActuationMode::Velocity;
    }
    if (v == "torque" || v == "effort" || v == "force") {
        return MujocoJointActuationMode::Torque;
    }
    if (v == "pd_torque" || v == "pd") {
        return MujocoJointActuationMode::Position;
    }
    if (v == "direct" || v == "kinematic" || v == "qpos") {
        return MujocoJointActuationMode::Direct;
    }
    RCLCPP_WARN(logger, "Unknown mujoco_joint_actuation '%s'; using position.", it->second.c_str());
    return MujocoJointActuationMode::Position;
}

char const* actuation_mode_label(MujocoJointActuationMode mode) {
    switch (mode) {
        case MujocoJointActuationMode::Position:
            return "position";
        case MujocoJointActuationMode::Velocity:
            return "velocity";
        case MujocoJointActuationMode::Torque:
            return "torque";
        case MujocoJointActuationMode::Direct:
            return "direct";
    }
    return "position";
}
...
hardware_interface::CallbackReturn FeixiMujocoHardware::on_init(
    ...
    arm_kv_ = parse_double_or(pmap, "arm_kv", arm_kd_);
    gripper_kp_ = parse_double_or(pmap, "gripper_ctrl_kp", 380.0);

    joint_actuation_mode_ = parse_joint_actuation_mode(get_logger(), pmap);
    auto cmd_it = pmap.find("command_interface");
    if (cmd_it != pmap.end() && !cmd_it->second.empty()) {
        std::string v;
        ...
        char const* expected = actuation_mode_label(joint_actuation_mode_);
        if (joint_actuation_mode_ == MujocoJointActuationMode::Torque) {
            expected = "effort";
        }
        if (v != expected) {
            RCLCPP_WARN(get_logger(),
                        "command_interface=%s does not match mujoco_joint_actuation=%s (expected %s).",
                        cmd_it->second.c_str(), actuation_mode_label(joint_actuation_mode_), expected);
        }
    }
    ...
    if (pinocchio_mode_ != PinocchioDynamicsMode::None) {
        if (joint_actuation_mode_ == MujocoJointActuationMode::Direct) {
            RCLCPP_ERROR(get_logger(),
                         "dynamics_mode is incompatible with mujoco_joint_actuation=direct.");
            free_mujoco();
            return hardware_interface::CallbackReturn::ERROR;
        }
        if (joint_actuation_mode_ != MujocoJointActuationMode::Position) {
            RCLCPP_ERROR(get_logger(),
                         "dynamics_mode requires mujoco_joint_actuation=position (pd_torque).");
            free_mujoco();
            return hardware_interface::CallbackReturn::ERROR;
        }
    ...
    RCLCPP_INFO(get_logger(), "mujoco_joint_actuation=%s.", actuation_mode_label(joint_actuation_mode_));
...
hardware_interface::return_type FeixiMujocoHardware::write(const rclcpp::Time&,
    ...
    std::array<double, 8> v_cmd{};
    v_cmd.fill(0.0);

    bool const accept_commands = init_lock_writes_remaining_ <= 0;
    if ((joint_actuation_mode_ == MujocoJointActuationMode::Position ||
         joint_actuation_mode_ == MujocoJointActuationMode::Direct) &&
        accept_commands) {
        for (int i = 0; i < 7; ++i) {
    ...
    } else if (joint_actuation_mode_ == MujocoJointActuationMode::Velocity && accept_commands) {
        for (int i = 0; i < 7; ++i) {
            std::string const jkey = "joint" + std::to_string(i + 1);
            std::string const vel_iface = jkey + "/" + hardware_interface::HW_IF_VELOCITY;
            if (has_command(vel_iface)) {
                double const cmd = get_command<double>(vel_iface);
                if (std::isfinite(cmd)) {
                    v_cmd[static_cast<std::size_t>(i)] = cmd;
                }
            }
        }
        {
    ...
    bool const use_pinocchio =
        (joint_actuation_mode_ == MujocoJointActuationMode::Position &&
         pinocchio_mode_ != PinocchioDynamicsMode::None && pinocchio_dyn_ != nullptr &&
         pinocchio_dyn_->ok());

    std::optional<Eigen::VectorXd> tau_pin;

    bool const use_direct = joint_actuation_mode_ == MujocoJointActuationMode::Direct;
    ...
    if (use_pinocchio) {
        ...
        for (int i = 0; i < 7; ++i) {
            data_->ctrl[mj_arm_act_ids_[i]] = 0.0;
        }
        {
            int const aid = mj_gripper_act_;
    ...
    } else if (joint_actuation_mode_ == MujocoJointActuationMode::Torque) {
        for (int i = 0; i < 7; ++i) {
            int const aid = mj_arm_act_ids_[i];
            double lo = model_->actuator_ctrlrange[2 * aid];
            double hi = model_->actuator_ctrlrange[2 * aid + 1];
            double tau = 0.0;
            std::string const eff_iface =
                "joint" + std::to_string(i + 1) + "/" + hardware_interface::HW_IF_EFFORT;
            if (accept_commands && has_command(eff_iface)) {
        ...
        }
        {
            ...
            if (accept_commands && has_command(g_eff)) {
                double const c = get_command<double>(g_eff);
                tau = std::isfinite(c) ? c : 0.0;
            }
            data_->ctrl[aid] = mju_clip(tau, lo, hi);
        }
    } else if (joint_actuation_mode_ == MujocoJointActuationMode::Velocity) {
        for (int i = 0; i < 7; ++i) {
            int const jid = mj_arm_joint_ids_[i];
            int const aid = mj_arm_act_ids_[i];
            int const va = model_->jnt_dofadr[jid];
            ...
            double lo = model_->actuator_ctrlrange[2 * aid];
            double hi = model_->actuator_ctrlrange[2 * aid + 1];
            double const v_des = v_cmd[static_cast<std::size_t>(i)];
            double tau = arm_kv_ * (v_des - qdot);
            if (!std::isfinite(tau)) {
                tau = 0.0;
            }
            data_->ctrl[aid] = mju_clip(tau, lo, hi);
        ...
        {
            int const gdl = model_->jnt_dofadr[mj_gripper_joint_];
            double gqdot = data_->qvel[gdl];
            int const aid = mj_gripper_act_;
            double lo = model_->actuator_ctrlrange[2 * aid];
            double hi = model_->actuator_ctrlrange[2 * aid + 1];
            double tau = arm_kv_ * (v_cmd[7] - gqdot);
            if (!std::isfinite(tau)) {
                tau = 0.0;
            }
            data_->ctrl[aid] = mju_clip(tau, lo, hi);
        ...
        for (int i = 0; i < 7; ++i) {
            ...
            double tau = arm_kp_ * (q_des - q) - arm_kd_ * qdot;
            if (!std::isfinite(tau)) {
                tau = 0.0;
            }
            data_->ctrl[aid] = mju_clip(tau, lo, hi);
...
PLUGINLIB_EXPORT_CLASS(feixi_mujoco_control::FeixiMujocoHardware, hardware_interface::SystemInterface)
